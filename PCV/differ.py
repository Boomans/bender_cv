...
HS_Cascade = cv2.CascadeClassifier('PCV/HS.xml')
img_cr = None
...
def diff(img_cur, img_empt):
    ...
    p2 = Process(target=calc_dif,
                 args=(
                 img_cur[len(img_cur) // 4: len(img_cur) // 2, :], img_empt[len(img_cur) // 4: len(img_cur) // 2, :],
                 result_queue, 2))
    p3 = Process(target=calc_dif,
                 args=(img_cur[len(img_cur) // 2: len(img_cur) // 4 * 3, :],
                       img_empt[len(img_cur) // 2: len(img_cur) // 4 * 3, :], result_queue, 3))
    p4 = Process(target=calc_dif,
                 args=(img_cur[len(img_cur) // 4 * 3:, :], img_empt[len(img_cur) // 4 * 3:, :], result_queue, 4))
